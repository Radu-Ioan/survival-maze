...
namespace implemented
{
    class CameraCorect
    {
     public:
        ...
        CameraCorect(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
        {
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            /* old (wrong) */
            // buffer = glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 0, 1))
            //             * glm::vec4(right, 1);
            /* correct */
            ...
            /* analog as above was made the same mistake, but the current below
               is correct*/
            buffer = glm::rotate(glm::mat4(1.0f), angle, forward)
                        * glm::vec4(up, 1);
            up = glm::normalize(glm::vec3(buffer));
    ...
    private:
        void wrongRotateThirdPerson_OX(float angle)
        {
            // [DONE]: Rotate the camera in third-person mode around
            // the OX axis. Use `distanceToTarget` as translation distance.
            TranslateForward(distanceToTarget);
        ...
        void wrongRotateThirdPerson_OY(float angle)
        {
            // [DONE]: Rotate the camera in third-person mode around
            // the OY axis.
            TranslateForward(distanceToTarget);
        ...
        void wrongRotateThirdPerson_OZ(float angle)
        {
            // [DONE]: Rotate the camera in third-person mode around
            // the OZ axis.
            TranslateForward(distanceToTarget);
